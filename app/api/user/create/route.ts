...
export async function POST(request: Request): Promise<Response> {
  try {
    ...
    if (existing) {
      return new Response(JSON.stringify({ valid: false, message: 'username already exists', code: '409' }), {
        status: 409,
        headers: { 'Content-Type': 'application/json' },
      });
    }
    ...
    return new Response(
      JSON.stringify({ valid: true, message: 'user created successfully', code: '201', data: { id: user.id, username: user.name, createdAt: user.createdAt } }),
      { status: 201, headers: { 'Content-Type': 'application/json' } },
    );
  } catch (err) {
    console.error(err);
    const msg = err instanceof Error ? err.message : String(err);
    return new Response(JSON.stringify({ valid: false, message: msg, code: '500' }), {
      status: 500,
      headers: { 'Content-Type': 'application/json' },
    });
  }
